...
export function increment(str: string, options: any) {
  init(options);

  if (str == undefined) return getFirst();
...
export function decrement(str: string, options: any) {
  init(options);
...
function same(str: string, char: any) {
  var i = str.length;
